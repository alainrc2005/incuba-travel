...
<main>
    ...
    <section>
        <div class="container">
            <div class="hr-title">CONFIRMACIÓN SOLICITUD DE AUTO</div>
            <div class="row">
                <div class="col-md-offset-2 col-md-8">
                    ...
                    <div class="row">
                        <div class="form-group">
                            <label class="control-label col-md-5" for="observations">Observaciones<span
                                        class="require"> *</span></label>
                            <div class="col-md-7">
                            <textarea id="observations" class="form-control"
                                      data-bind="textInput: dsend.observations"></textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row">
...
<script type="text/javascript">
    function vmCarConfirmation() {
        ...
        self.dsend = {
            ...
            observations: ko.observable().extend({required: true}),
            ict_payid: ko.observable("<?=$Payment->ict_payid?>")
        };
        self.errors = ko.validation.group(self.dsend);
        self.sendConfirmation = function () {
            App.ACmd({
                url: "<?=base_url('frontend/Home/sendCarConfirmation')?>",
                data: ko.toJS(self.dsend)
            }).done(function (response) {
                switch (response) {
                    case "Ok":
                        App.ict_Info("La confirmación y el envío de mensaje al cliente se completaron");
                        setTimeout(function () {
                            window.location = "<?=base_url('Home')?>";
                        }, 3000);
                        break;
                    case "ErrorApiKey":
                        App.ict_Error("Error en llave de acceso para confirmar solicitud");
                        break;
                    case "ErrorDB":
                        App.ict_Error("Error confirmando solicitud de auto: DB");
                        break;
                    case "ErrorMail":
    ...
    $(document).ready(function () {
        ko.applyBindings(new vmCarConfirmation());
    });
</script>
</body>
</html>